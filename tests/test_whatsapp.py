...
pytestmark = pytest.mark.skip(
    reason="Service has duplicated metrics - needs service fix"
)
...
def test_signature_invalid_returns_401():
    ...
    assert (
        ...
        or "Timestamp" in response.text
        or "format" in response.text
    )
...
def test_signature_valid_forwards_to_orchestrator(mock_post):
    ...
    response = client.post(
        "/webhook/ycloud", content=body, headers={"ycloud-signature": sig}
    )

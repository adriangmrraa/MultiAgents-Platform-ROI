...
pytestmark = pytest.mark.skip(
    reason="Test broken - patches don't match current service architecture"
)
...
with (
    ...
    patch("orchestrator_service.main.db", new_callable=MagicMock) as mock_db_instance,
):  # Mocking the 'db' IMPORTED in main
    ...
    @patch(
        "orchestrator_service.main.db.mark_inbound_processing", new_callable=AsyncMock
    )
    ...
    def test_dedupe_option_a(
        ...
        mock_agent_instance.ainvoke = AsyncMock(
            return_value={"output": "Response from agent"}
        )

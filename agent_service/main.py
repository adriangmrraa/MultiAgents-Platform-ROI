...
class OrchestratorResponse(BaseModel):
    messages: List[OrchestratorMessage] = Field(
        description="List of messages to send to the user."
    )
...
class AgentConfig(BaseModel):
    ...
    wizard_overrides: Optional[
        Dict[str, Any]
    ] = {}  # Nexus v5.27: Stores "tone", "business_rules", etc.
    temperature: Optional[float] = None  # Nexus v5.99: Dynamic creativity control
    reasoning_effort: Optional[str] = (
        None  # Nexus v5.99: GPT-5.2 advanced param (none/low/medium/high/xhigh)
    )
...
ctx_user_id: ContextVar[str] = ContextVar(
    "ctx_user_id", default=""
)  # Strict Isolation (v5.10)
...
def prune_history(
    history: List[Any], max_tokens: int = 4000, model: str = "gpt-4o-mini"
) -> List[Any]:
    """
    ...
    try:
        ...
        logger.warning("tiktoken_model_not_found", model=model, error=str(e))
    ...
    filtered_history = [
        ...
        or (isinstance(m, dict) and m.get("content", "").strip())
    ]
...
async def browse_general_storefront():
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            resp = await client.post(
                f"{ctx_service_url.get()}/tools/productsall",
                json=payload,
                headers=headers,
            )
            logger.info(
                "tool_call_response",
                tool="browse_general_storefront",
                status=resp.status_code,
            )
            if resp.status_code == 200:
                data = resp.json()
...
async def search_by_category(category: str, keyword: str = ""):
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            resp = await client.post(
                f"{ctx_service_url.get()}/tools/productsq_category",
                json=payload,
                headers=headers,
            )
            logger.info(
                "tool_call_response", tool="search_by_category", status=resp.status_code
            )
            if resp.status_code == 200:
                data = resp.json()
                if data.get("ok"):
...
async def cupones_list():
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            resp = await client.post(
                f"{ctx_service_url.get()}/tools/cupones_list",
                json=payload,
                headers=headers,
            )
            logger.info(
                "tool_call_response", tool="cupones_list", status=resp.status_code
            )
            if resp.status_code == 200:
                data = resp.json()
                if data.get("ok"):
...
async def orders(q: str):
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            resp = await client.post(
                f"{ctx_service_url.get()}/tools/orders", json=payload, headers=headers
            )
...
async def search_knowledge_base(q: str):
    ...
    headers = {
        "X-Internal-Secret": ctx_internal_token.get(),
        "x-admin-token": os.getenv("ADMIN_TOKEN", ""),
    }
        ...
        try:
            ...
            try:
                resp = await client.get(
                    f"{orch_url}/admin/rag/search", params=params, headers=headers
                )
            except Exception as e:
                ...
                if "Name or service not known" in str(e) or "ConnectError" in str(e):
                    if "orchestrator_service" in orch_url:
                        dash_url = orch_url.replace(
                            "orchestrator_service", "orchestrator-service"
                        )
                        logger.warning(
                            f"🔁 RAG CONFIG: Primary host failed. Attempting Dash Fallback | url={dash_url}"
                        )
                        resp = await client.get(
                            f"{dash_url}/admin/rag/search",
                            params=params,
                            headers=headers,
                        )
                    else:
...
async def report_assistance(type: str, score: float, reasoning: str):
    ...
    orch_url = os.getenv("ORCHESTRATOR_URL", "http://orchestrator_service:8000")
    headers = {
        "X-Internal-Token": ctx_internal_token.get(),
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            resp = await client.post(
                f"{orch_url}/admin/tools/report_assistance",
                json=payload,
                headers=headers,
            )
            if resp.status_code == 200:
            ...
            try:
                ...
                if resp_shadow.status_code == 200:
                    ...
                    if data.get("ok") and data.get("results"):
                        ...
                        shadow_block = "\n".join(
                            [
                                f"- [{item['metadata'].get('timestamp', '?')}]: {item['content']}"
...
async def execute_agent(
    ...
    x_internal_secret: str = Header(
        None, alias="X-Internal-Secret"
    ),  # Protocol Omega: Header Handshake
):
    # Security Check
    ...
    ctx_token.set(
        request.credentials.tiendanube_access_token.get_secret_value()
        if request.credentials.tiendanube_access_token
        else ""
    )
    ctx_service_url.set(request.credentials.tiendanube_service_url)
    ctx_internal_token.set(x_internal_secret or "")
    ctx_knowledge_sources.set(
        request.agent_config.knowledge_sources if request.agent_config else []
    )
    ...
    logger.info(
        f"🔧 Tools Context | store_id={ctx_store_id.get() or 'EMPTY'} | token={'***' if ctx_token.get() else 'EMPTY'} | service_url={ctx_service_url.get()}"
    )
    ...
    if last_user_msg:
        hybrid_context_block = await retrieve_context(
            ...
            headers=headers,
            orch_url=orch_url,
            shadow_enabled=getattr(request.agent_config, "shadow_rag_enabled", False)
    ...
    if (
        ...
        and request.agent_config.model
        and "name" in request.agent_config.model
    ):
    ...
    if provider == "openai":
        resolved_api_key = request.credentials.openai_api_key or os.getenv(
            "OPENAI_API_KEY"
        )
    else:
        resolved_api_key = request.credentials.google_api_key or os.getenv(
            "GOOGLE_API_KEY"
        )

    if not resolved_api_key:
        logger.warning(
            "agent_execution_paused_missing_key",
            provider=provider,
            tenant_id=request.tenant_id,
        )
        raise HTTPException(
    ...
    if model_name in ["gpt-5.2", "gpt-5.2-pro"]:
        ...
        logger.info(
            "gpt52_reasoning_effort_set", model=model_name, effort=reasoning_effort
        )
    ...
    if (
        ...
        and len(request.context.system_prompt.strip()) > 10
    ):
        final_system_prompt += (
            f"\n\n## ADDITIONAL CONTEXT\n{request.context.system_prompt}"
        )
    ...
    prompt = ChatPromptTemplate.from_messages(prompt_msgs).partial(
    ...
    async def event_generator():
        try:
            ...
            async for event in executor.astream_events(
                {"input": request.message, "chat_history": history}, version="v1"
            ):
                kind = event["event"]

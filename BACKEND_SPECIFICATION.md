# Nexus v5 Backend Technical Specification (Protocol Omega)
...
### 🚀 Protocol Omega Streaming (Real-Time Assets)
The "Heart" of Nexus v5. The frontend subscribes to this stream to receive assets generated by the `engine`.

**`GET /admin/engine/stream/{tenant_id}`**
- **Auth**: Query Param `?token={ADMIN_TOKEN}` (EventSource limitation).
- **Mechanism**: Server-Sent Events (SSE) proxied from Redis Pub/Sub.
- **Channel Isolation**: Subscribes to Redis channel `events:tenant:{tenant_id}:assets`.

**Event Protocol (Strict JSON):**
1.  **`asset_generated`**: When a new artifact is ready.
    ```json
    {
      "event": "asset_generated",
      "data": {
        "asset_id": "uuid",
        "asset_type": "visuals", // branding | scripts | visuals | roi
        "content": { ... }       // JSON Payload
      }
    }
    ```
2.  **`task_completed`**: When the full ignition cycle ends.
    ```json
    {
...
### `business_assets` (PostgreSQL) - Protocol Omega SSOT
The persistent store for all generated business intelligence.

| Column | Type | Description |
| :--- | :--- | :--- |
| `id` | UUID | Primary Key |
| `tenant_id` | Text | Tenant Identifier (Phone/Store ID) |
| `asset_type` | Varchar | `branding`, `scripts`, `visuals`, `roi` |
| `content` | JSONB | The actual generated data (SSOT) |
...
| `phone_number` | Varchar | **Nullable**. Unique per Tenant. |

### `agents` (PostgreSQL) - v5.0
| Column | Type | Description |
| :--- | :--- | :--- |
...
### Error Boundaries
- **500 Internal Server Error**: Check `admin_routes.py` and `engine.py` logs.

...
export const RagGalaxy: React.FC = () => {
    ...
    // Simulating "Learned Knowledge" (In real app, this comes from /api/rag/vectors)
    useEffect(() => {
        const categories = [
            { name: 'Products', color: '#cyan' },
            { name: 'Policy', color: '#purple' },
            { name: 'Brand', color: '#green' }
        ];
        ...
        for (let i = 0; i < 50; i++) {
            ...
            newNodes.push({
                ...
                category: cat.name,
                color: cat.color === '#cyan' ? 'rgba(34, 211, 238, 0.8)' :
                    cat.color === '#purple' ? 'rgba(168, 85, 247, 0.8)' : 'rgba(74, 222, 128, 0.8)',
                description: `Neural encoded vector for ${cat.name} #${i}. Semantic proximity to user intent high.`,
                meta: `DIM: 1536 | SIM: 0.9${Math.floor(Math.random() * 9)}`
            });
        }
        setNodes(newNodes);
    }, []);

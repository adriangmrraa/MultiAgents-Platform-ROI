            ...
            {stats?.roi_metrics && (
                <div className="mb-8">
                    <div className="glass p-8 rounded-2xl border border-emerald-500/20 bg-emerald-950/10 relative overflow-hidden">
                        ...
                        <h2 className="text-sm font-bold text-emerald-400 tracking-wider mb-2">{t('dashboard.valueGenerated')}</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="stat-card glass">
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span className="stat-label">{t('dashboard.commsTraffic')}</span>
                        <MessageSquare className="stat-icon" size={20} color="var(--text-secondary)" />
                    </div>
                    <span className="stat-value">{stats?.total_messages || 0}</span>
                    <span className="stat-meta">{t('dashboard.interactions')}</span>
                </div>

                {/* Efficiency */}
                <div className="stat-card glass">
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span className="stat-label">{t('dashboard.efficiency')}</span>
                    ...
                    <span className="stat-meta">{t('dashboard.successRate')}</span>
                </div>

                {/* System Status (Condensed) */}
                <div className="glass p-4 rounded-xl flex items-center justify-between">
                    <div>
                        <div className="text-xs text-white/40 uppercase font-bold tracking-wider">{t('dashboard.systemHealth')}</div>

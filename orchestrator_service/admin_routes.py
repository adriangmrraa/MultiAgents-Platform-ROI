...
async def verify_admin_token(x_admin_token: str = Header(None)):
    if x_admin_token != ADMIN_TOKEN:
        ...
        print(
            f"AUTH_DEBUG: Expected '{masked_expected}' vs Received '{masked_received}'"
        )
...
async def verify_internal_token(
    ...
    if not INTERNAL_API_TOKEN:
        raise HTTPException(
            status_code=500, detail="Security Config Missing (INTERNAL_API_TOKEN)"
        )
...
@router.get(
    "/internal/credentials/{name}", dependencies=[Depends(verify_internal_token)]
)
async def get_internal_credential_endpoint(name: str, tenant_id: Optional[int] = None):
    ...
    try:
        ...
        if val:
            logger.info(
                f"✅ credential_fetched: {name} | length={len(val) if val else 0}"
            )
...
async def create_tool(tool: ToolCreate, current_user: User = Depends(get_current_user)):
    try:
        ...
        # Determine Tenant ID
        tenant_id = (
            current_user.tenant_id if current_user.role != "SuperAdmin" else None
        )  # Admin makes global
...
async def ensure_ycloud_credential_types():
    ...
    try:
        ...
        # 2. Get Provider ID
        provider_id = await db.pool.fetchval(
            "SELECT id FROM oauth_providers WHERE name = 'whatsapp_cloud'"
        )
...
async def get_tenant_by_id(
    tenant_id: int, current_user: User = Depends(get_current_user)
):
    """
    ...
    if current_user.role != "SuperAdmin":
        ...
        owner_check = await db.pool.fetchval(
            "SELECT owner_email FROM tenants WHERE id = $1", tenant_id
        )
...
async def update_tenant(
    tenant_id: int, data: dict, current_user: User = Depends(get_current_user)
):
    """
    ...
    if "handoff_policy" in data:
        ...
        params.append(
            json.dumps(policy) if isinstance(policy, (dict, list)) else policy
        )
        param_idx += 1
...
async def delete_tenant(tenant_id: int):
    ...
    try:
        ...
        await db.log_system_event(
            "warning", "tenant_archived", f"Tenant {tenant_id} archived (Soft Delete)"
        )

        return {
            "status": "archived",
            "id": tenant_id,
            "message": "Tenant archived for safety.",
        }
...
async def unbind_channel(
    ...
    await db.pool.execute(
        "DELETE FROM channel_bindings WHERE id = $1 AND tenant_id = $2",
        binding_id,
        tenant_id,
    )
...
async def edit_channel(
    binding_id: int, request: Request, current_user: User = Depends(get_current_user)
):
    """
    ...
    if new_channel_id:
        ...
        if conflict:
            raise HTTPException(
                409, detail=f"Channel ID {new_channel_id} is already bound"
            )
...
async def resolve_tenant_from_channel(provider: str, channel_id: str):
    ...
    try:
        ...
        logger.warning("redis_cache_write_failed", extra={"error": str(e)})
...
async def delete_tool(name: str, current_user: User = Depends(get_current_user)):
    ...
    if name in system_tool_names:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete a system-level tool. You can only customize its instructions via Tool Config.",
        )
    ...
    if current_user.role == "SuperAdmin":
        ...
        result = await db.pool.execute(
            "DELETE FROM tools WHERE name = $1 AND tenant_id = $2", name, tenant_id
        )
...
@router.get(
    "/tenants/{tenant_id}/tools/config", dependencies=[Depends(get_current_user)]
)
async def get_tenant_tool_config(
    ...
    config = await db.pool.fetchval(
        "SELECT tool_config FROM tenants WHERE id = $1", tenant_id
    )
    return config or {}
...
async def update_tenant_tool_config(
    tenant_id: int, request: Request, current_user: User = Depends(get_current_user)
):
    # Security Check
    if current_user.role != "SuperAdmin" and current_user.tenant_id != tenant_id:
        raise HTTPException(403, "Access denied to other tenant settings")
    ...
    try:
        ...
        await db.pool.execute(
            "UPDATE tenants SET tool_config = $1 WHERE id = $2",
            json.dumps(data),
            tenant_id,
        )
        return {"status": "ok"}
...
async def save_credential(
    cred: CredentialModel, current_user: User = Depends(get_current_user)
):
    try:
        ...
        is_identifier = (
            cred.name.endswith("_ID")
            or cred.name.endswith("_URL")
            or "STORE_ID" in cred.name
            or cred.name.endswith("_URI")
        )
        ...
        if cred.scope == "tenant":
            ...
            existing = await db.pool.fetchval(
                "SELECT id_uuid FROM credentials WHERE name=$1 AND tenant_id=$2 AND scope='tenant'",
                cred.name,
                cred.tenant_id,
            )
            if existing:
                await db.pool.execute(
                    "UPDATE credentials SET value=$1, category=$2, description=$3, updated_at=NOW() WHERE id_uuid=$4",
                    final_value,
                    cred.category,
                    cred.description,
                    existing,
        ...
        if cred.tenant_id:
            try:
                if cred.name == "TIENDANUBE_ACCESS_TOKEN":
                    await db.pool.execute(
                        "UPDATE tenants SET tiendanube_access_token = $1 WHERE id = $2",
                        final_value,
                        cred.tenant_id,
                    )
                elif cred.name == "TIENDANUBE_STORE_ID":
                    await db.pool.execute(
                        "UPDATE tenants SET tiendanube_store_id = $1 WHERE id = $2",
                        final_value,
                        cred.tenant_id,
                    )
...
async def delete_credential(
    cred_id: str, current_user: User = Depends(get_current_user)
):
    try:
        tenant_id = current_user.tenant_id
        user_role = (current_user.role or "owner").lower()
        ...
        row = await db.pool.fetchrow(
            "SELECT category, tenant_id, scope FROM credentials WHERE id_uuid = $1",
            uuid.UUID(cred_id),
        )
        if not row:
        ...
        await db.pool.execute(
            "DELETE FROM credentials WHERE id_uuid = $1 AND tenant_id = $2",
            uuid.UUID(cred_id),
            row["tenant_id"],
        )
...
async def internal_credential_sync(
    data: MetaSyncRequest, x_internal_secret: str = Header(None)
):
    """
    ...
    try:
        ...
        async def save_credential(
            name: str, value: str, category: str, internal_key: str = None
        ):
            ...
            if internal_key:
                type_id = await db.pool.fetchval(
                    "SELECT id FROM credential_types WHERE internal_key = $1",
                    internal_key,
                )
        ...
        if data.provider == "meta":
            ...
            if user_token:
                await save_credential(
                    "META_USER_LONG_TOKEN", user_token, "meta", "META_USER_ACCESS_TOKEN"
                )
        ...
        if "pages" in assets:
            await store_asset_batch(assets["pages"], "facebook_page")
        if "instagram" in assets:
            await store_asset_batch(assets["instagram"], "instagram_account")
        if "whatsapp" in assets:
            await store_asset_batch(assets["whatsapp"], "whatsapp_waba")
...
async def update_channels_selection(
    data: ChannelSelectionRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        for row in rows:
            ...
            is_selected = (
                meta_external_id in data.selected_assets
                or str(row["id"]) in data.selected_assets
            )
            ...
            if is_selected:
                ...
                if asset_type == "facebook_page":
                    ...
                    token_row = await db.pool.fetchrow(
                        ...
                        tenant_id,
                        f"META_PAGE_TOKEN_{meta_external_id}",
                    )
                    if token_row:
                    ...
                    if token:
                        try:
                            ...
                            logger.info(
                                "webhook_subscribed_page", page_id=meta_external_id
                            )
                        except Exception as sub_err:
                            logger.warning(
                                "webhook_subscribe_failed_page",
                                page_id=meta_external_id,
                                error=str(sub_err),
                            )
                    ...
                    if linked_page_id:
                        ...
                        token_row = await db.pool.fetchrow(
                            "SELECT value FROM credentials WHERE tenant_id = $1 AND name = $2",
                            tenant_id,
                            f"META_PAGE_TOKEN_{linked_page_id}",
                        )
                        if token_row:
                            token = decrypt_password(token_row["value"])
                            try:
                                ...
                                logger.info(
                                    "webhook_subscribed_ig", ig_id=meta_external_id
                                )
                            except Exception as sub_err:
                                logger.warning(
                                    "webhook_subscribe_failed_ig", error=str(sub_err)
                                )
                    ...
                    token_row = await db.pool.fetchrow(
                        "SELECT value FROM credentials WHERE tenant_id = $1 AND name = $2",
                        tenant_id,
                        f"META_WA_TOKEN_{meta_external_id}",
                    )
                    if token_row:
                        token = decrypt_password(token_row["value"])
                        ...
                        for phone in phone_numbers:
                            try:
                                ...
                                logger.info(
                                    "webhook_subscribed_wa",
                                    waba_id=meta_external_id,
                                    phone=phone.get("display_phone_number"),
                                )
                            except Exception as sub_err:
                                logger.warning(
                                    "webhook_subscribe_failed_wa", error=str(sub_err)
                                )
...
async def list_credentials(
    ...
    try:
        ...
        logger.warning("redis_cache_read_failed", extra={"error": str(e)})
    ...
    try:
        ...
        for item in data:
            ...
            if item.get("category") in SENSITIVE_CATEGORIES and item.get("value"):
                ...
                if dec:
                    item["value"] = dec  # Only replace if decryption succeeded
...
async def ignite_engine(
    data: MagicOnboardingRequest, background_tasks: BackgroundTasks
):
    """
    ...
    provisional_phone = (
        data.bot_phone_number
        if data.bot_phone_number not in ["TBD", ""]
        else f"prov_{uuid.uuid4().hex[:8]}"
    )
    ...
    """
    row_tenant = await db.pool.fetchrow(
        q_tenant,
        data.store_name,
        provisional_phone,
        data.tiendanube_store_id,
        encrypted_token,
        data.store_url,
    )
    ...
    for template in standard_agents:
        # Idempotency check
        exists = await db.pool.fetchval(
            "SELECT 1 FROM agents WHERE tenant_id = $1 AND role = $2",
            tenant_id,
            template["role"],
        )
        if not exists:
...
async def run_rag_ingestion(tenant_id: int, identifier: str, store_id: str, token: str):
    ...
    try:
        ...
        headers = {
            ...
            "User-Agent": "Nexus Bot (nexus@platform.com)",
        }
                ...
                products = [
                    {
                        ...
                        "description": {"es": "Producto autogenerado por Nexus Magic"},
                        "price": "100.00",
                    },
                    {
                        "id": 992,
                        "name": {"es": "Pantalón Demo Magic"},
                        "description": {"es": "Calidad premium detectada"},
                        "price": "200.00",
                    },
                ]
        ...
        await db.pool.execute(
            "INSERT INTO system_events (event_type, severity, message, tenant_id, occurred_at) VALUES ('rag_failed', 'ERROR', $1, $2, NOW())",
            str(e),
            tenant_id,
        )
...
async def generate_ad_image(request: Request):
    ...
    try:
        ...
        return {
            "status": "success",
            "url": generated_url,
            "description": "Reference-based image generation (Nano Banana)",
        }
...
async def get_business_assets(
    asset_type: Optional[str] = None, current_user: User = Depends(get_current_user)
):
    """
...
async def test_credential_connection(
    data: ConnectionTestRequest, current_user: User = Depends(get_current_user)
):
    """
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    name = store_data.get("name", {}).get("es") or store_data.get(
                        "name"
                    )
                    return {
                        "status": "ok",
                    ...
                    logger.error(
                        "tn_auth_failed", status=resp.status_code, body=resp.text
                    )
                    ...
                    return {
                        "status": "error",
                        "message": f"Unauthorized (Token Invalid/Forbidden) {resp.status_code}",
                        "valid": False,
                    }
                elif resp.status_code >= 500:
                    # Handle server errors
                    logger.error(
                        "tn_server_error", status=resp.status_code, body=resp.text
                    )
                    return {
                        "status": "error",
                        "message": f"Upstream Server Error {resp.status_code}",
                        "valid": False,
                    }
                else:
                    return {
...
async def get_store_products(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if not token:
            row = await db.pool.fetchrow(
                "SELECT tiendanube_access_token FROM tenants WHERE id = $1", tenant_id
            )
        ...
        if not token or not store_id:
            ...
            return [
                {
                    "id": 991,
                    "name": {"es": "Camiseta CyberPunk Gen1"},
                    "images": [
                        {
                            "src": "https://via.placeholder.com/300/000000/00ffff?text=CyberTee"
                        }
                    ],
                    "categories": [{"id": 1, "name": {"es": "Ropa"}}],
                },
                {
                    "id": 992,
                    "name": {"es": "Zapatillas Neon Runner"},
                    "images": [
                        {
                            "src": "https://via.placeholder.com/300/111111/ff00ff?text=NeonShoes"
                        }
                    ],
                    "categories": [{"id": 1, "name": {"es": "Calzado"}}],
                },
            ]
        ...
        headers = {
            "Authentication": f"bearer {token}",
            "User-Agent": "Nexus Bot (Platform AI)",
        }
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                logger.warning(
                    "products_fetch_401_token_expired_despite_refresh_attempt"
                )
                return []
            else:
    ...
    async def event_generator():
        yield {
            ...
            "data": json.dumps(
                {
                    "event_type": "info",
                    "message": f"Connected to Protocol Omega Stream for {tenant_id}",
                }
            ),
        }
...
async def console_stream(request: Request, token: Optional[str] = None):
    ...
    async def event_generator():
        ...
        try:
            ...
            logger.warning("system_events_max_id_fetch_failed", extra={"error": str(e)})
...
async def bootstrap():
    ...
    last_inbound = await db.pool.fetchval(
        "SELECT MAX(created_at) FROM chat_messages WHERE role = 'user'"
    )
    last_outbound = await db.pool.fetchval(
...
async def get_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if (
            sales_metrics["tiendanube_connected"]
            and sales_metrics["total_revenue_30d"] > 0
        ):
...
async def _fetch_tn_orders_summary(tenant_id: int, days: int = 30) -> Optional[Dict]:
    ...
    if not store_id:
        store_id_raw = await db.pool.fetchval(
            "SELECT tiendanube_store_id FROM tenants WHERE id = $1", tenant_id
        )
        if store_id_raw:
            from utils import decrypt_password
...
async def get_sales_dashboard(
    ...
    result = {
        ...
        "attributed_to_ai": {
            ...
            "percentage": round(
                (attributed["orders"] / total_orders * 100) if total_orders > 0 else 0,
                1,
            ),
        },
    }
...
async def get_sales_orders(
    ...
    if status:
        all_orders = [
            o
            for o in all_orders
            if o.get("status") == status or o.get("payment_status") == status
        ]
...
async def admin_ops(action: str, payload: dict = {}):
    ...
    if action == "clear_cache":
        ...
        if not pattern.startswith("dashboard:") and not pattern.startswith("cache:"):
            if (
                pattern != "*"
            ):  # Allow full clear if explicitly requested by SuperAdmin? Let's limit for now.
        ...
        if not conversation_id:
            ...
            if phone and tenant_id:
                row = await db.pool.fetchrow(
                    "SELECT id FROM chat_conversations WHERE external_user_id = $1 AND tenant_id = $2",
                    phone,
                    tenant_id,
                )
                if row:
        ...
        await db.pool.execute(
            "UPDATE chat_conversations SET human_override_until = $1, status = 'human_override' WHERE id = $2",
            lock_until,
            conversation_id,
        )
        ...
        config = await db.pool.fetchrow(
            "SELECT * FROM tenant_human_handoff_config WHERE tenant_id = $1", tenant_id
        )
        if not config:
            return {
                "status": "ok",
                "message": "Handoff triggered (AI Paused), but email NOT sent (No Config found).",
            }
        ...
        for r in history_rows:
            ...
            sender = (
                "BOT"
                if r["role"] == "assistant"
                else f"USER ({r['from_number'] or 'Client'})"
            )
        ...
        try:
            ...
            msg["Subject"] = (
                f"🚨 Handoff Request: {conv['external_user_id']} ({conv['channel']})"
            )
            ...
            return {
                "status": "ok",
                "message": f"Handoff triggered. AI paused for 24h. Email sent to {config['destination_email']}.",
            }
            ...
            return {
                "status": "warning",
                "message": f"AI Paused, but Email failed: {str(e)}",
            }
...
async def get_available_models():
    ...
    return {"default_model": DEFAULT_MODEL, "models": MODEL_REGISTRY}
...
async def list_chats(
    ...
    if human_override is not None:
        if human_override:
            ...
            where_clauses.append(
                f"(human_override_until IS NULL OR human_override_until <= NOW())"
            )
    ...
    try:
            ...
            results.append(
                {
                    ...
                    "timestamp": r["last_message_at"].isoformat()
                    if r["last_message_at"]
                    else None,
        ...
        logger.info(
            f"Auditing list_chats: Returning {len(results)} conversations (Limit: {limit}, Offset: {offset})"
        )
        return results
...
async def get_chats_summary(
    ...
    logger.info(
        f"CHATS DEBUG: Querying tenant_id={current_user.tenant_id}, channel={channel}"
    )
    try:
    ...
    return [
        {
            ...
            "human_override_until": r["human_override_until"].isoformat()
            if r["human_override_until"]
            else None,
            "meta": json.loads(r["meta"])
            if r["meta"] and isinstance(r["meta"], str)
            else (r["meta"] if r["meta"] else {}),
        }
        for r in rows
    ]
...
async def get_chat_history(
    ...
    try:
        ...
        logger.warning("invalid_conversation_uuid", extra={"error": str(e)})
    ...
    for r in rows:
        ...
        if r["storage_url"]:
            media_obj = {
                "url": r["storage_url"]
                if r["storage_url"].startswith("http")
                else f"/admin/media/{r['media_id']}",  # Fallback logic
                "type": r["media_type"],
                "mime": r["mime_type"],
                "name": r["file_name"],
            }
...
@router.post(
    "/conversations/{conversation_id}/human-override",
...
async def get_handoff_config(tenant_id: int):
    ...
    data["smtp_password"] = "********"
    data["triggers"] = (
        json.loads(data["triggers"])
        if isinstance(data["triggers"], str)
        else data["triggers"]
    )
    data["email_context"] = (
        json.loads(data["email_context"])
        if isinstance(data["email_context"], str)
        else data["email_context"]
    )
    return data
...
async def upsert_handoff_config(config: HandoffConfigModel):
    existing = await db.pool.fetchrow(
        "SELECT smtp_password_encrypted FROM tenant_human_handoff_config WHERE tenant_id = $1",
        config.tenant_id,
    )
    ...
    if config.smtp_password == "********":
        if existing:
            ...
            raise HTTPException(
                status_code=400, detail="Password required for new configuration"
            )
    else:
    ...
    if config.smtp_host:
        config.smtp_host = (
            str(config.smtp_host).strip().replace("http://", "").replace("https://", "")
        )

    # Manual Upsert to avoid "InvalidColumnReferenceError" if constraints are missing or duplicated
    # 1. Check if exists
    existing = await db.pool.fetchrow(
        "SELECT 1 FROM tenant_human_handoff_config WHERE tenant_id = $1",
        config.tenant_id,
    )
    ...
    if existing:
        ...
        await db.pool.execute(
            ...
            password_to_store,
            json.dumps(config.triggers),
            json.dumps(config.email_context),
        )
    else:
        # INSERT
        q = """
...
async def create_tenant(tenant: TenantModel):
    ...
    tenant_id = await db.pool.fetchval(
        q,
        tenant.store_name,
        tenant.bot_phone_number,
        tenant.owner_email,
        tenant.store_location,
        tenant.store_website,
        tenant.store_description,
        tenant.store_catalog_knowledge,
        tenant.tiendanube_store_id,
        tenant.tiendanube_access_token,
        tenant.handoff_enabled,
        tenant.handoff_instructions,
        tenant.handoff_target_email,
        tenant.handoff_message,
        tenant.handoff_smtp_host,
        tenant.handoff_smtp_user,
        tenant.handoff_smtp_pass,
        tenant.handoff_smtp_port,
        json.dumps(tenant.handoff_policy or {}),
    )
...
async def get_tenant(phone: str):
    row = await db.pool.fetchrow(
        "SELECT * FROM tenants WHERE bot_phone_number = $1", phone
    )
    if not row:
...
async def delete_all_tenants():
    ...
    try:
        ...
        return {
            "status": "ok",
            "message": "System Factory Reset Complete (All Data Wiped)",
        }
    except Exception as e:
...
async def delete_tenant(identifier: str):
    ...
    if identifier.isdigit() and len(identifier) < 9:
        row = await db.pool.fetchrow(
            "SELECT id FROM tenants WHERE id = $1", int(identifier)
        )
        if row:
            tenant_id = row["id"]
    ...
    if not tenant_id:
        row = await db.pool.fetchrow(
            "SELECT id FROM tenants WHERE bot_phone_number = $1", identifier
        )
        if row:
            tenant_id = row["id"]
    ...
    if not tenant_id:
        ...
        row = await db.pool.fetchrow(
            "SELECT id FROM tenants WHERE bot_phone_number = $1", clean
        )
        if row:
            tenant_id = row["id"]
    ...
    if not tenant_id:
        raise HTTPException(
            status_code=404, detail=f"Tenant not found with identifier: {identifier}"
        )
    ...
    try:
        async with db.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute(
                    "DELETE FROM tenant_human_handoff_config WHERE tenant_id = $1",
                    tenant_id,
                )
                ...
                await conn.execute(
                    "DELETE FROM credentials WHERE tenant_id = $1", tenant_id
                )
        ...
        try:
            ...
            while True:
                cursor, keys = await redis_client.scan(
                    cursor=cursor, match=f"tenant:{tenant_id}:*", count=100
                )
                if keys:
                    await redis_client.delete(*keys)
                if cursor == 0:
        ...
        return {
            "status": "success",
            "message": f"Tenant {tenant_id} and all related data deleted successfully.",
        }
...
async def get_tenant_details(id: int):
    ...
    creds = await db.pool.fetch(
        "SELECT * FROM credentials WHERE tenant_id = $1 OR scope = 'global'", id
    )

    # Get active meta assets
    has_meta_assets = await db.pool.fetchval(
        "SELECT EXISTS(SELECT 1 FROM business_assets WHERE tenant_id = $1 AND (content->>'active')::boolean = true LIMIT 1)",
        str(id),
    )
    ...
    resp = {
        ...
        "credentials": {"tenant_specific": [], "global_available": []},
    }
    ...
    for c in creds:
        ...
        if c["tenant_id"] == id:
            ...
            c["scope"] == "global"
        ):  # Explicitly check scope to avoid mixing other tenants' data if query failed (paranoid check)
    ...
    for c in (
        resp["credentials"]["tenant_specific"] + resp["credentials"]["global_available"]
    ):
    ...
    # Web Widget Status
    has_web_config = any(
        c["category"] == "web_widget" for c in resp["credentials"]["tenant_specific"]
    )
...
@router.post(
    "/tenants/{phone}/test-message", dependencies=[Depends(verify_admin_token)]
)
...
async def unified_message_delivery(
    ...
    conv_data = await db.pool.fetchrow(
        "SELECT provider, external_chatwoot_id, external_account_id FROM chat_conversations WHERE id = $1",
        conv_id,
    )
    cw_id = payload_data.get("external_chatwoot_id") or (
        conv_data["external_chatwoot_id"] if conv_data else None
    )
    cw_acc = payload_data.get("external_account_id") or (
        conv_data["external_account_id"] if conv_data else None
    )

    # Emergency Fallback (v6.2.10): If account_id is missing, try global credential
    if not cw_acc and cw_id:
        cw_acc = await db.pool.fetchval(
            "SELECT value FROM credentials WHERE tenant_id = $1 AND name = 'CHATWOOT_ACCOUNT_ID'",
            tenant_id,
        )
        logger.info(
            f"🔄 RELAY: Account ID fallback | acc={cw_acc} | tenant={tenant_id}"
        )

    # Resolve provider from conversation if available
    conv_provider = (
        conv_data["provider"] if conv_data and conv_data["provider"] else None
    )
    ...
    if conv_provider == "meta_direct":
        ...
        has_meta = await db.pool.fetchval(
            "SELECT 1 FROM credentials WHERE tenant_id = $1 AND name LIKE 'META_PAGE_TOKEN_%'",
            tenant_id,
        )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if res.status_code not in [200, 201]:
                logger.error(
                    f"❌ RELAY: Gateway error | status={res.status_code} | body={res.text}"
                )
                raise HTTPException(
                    res.status_code, f"Delivery Gateway Error: {res.text}"
                )
        except Exception as e:
            msg = str(e)
            # DYNAMIC DNS FALLBACK: If underscore host fails, try dash host (Easypanel Standard)
            if (
                "Name or service not known" in msg or "ConnectError" in msg
            ) and "whatsapp_service" in wa_url:
                dash_url = wa_url.replace("whatsapp_service", "whatsapp-service")
                logger.warning(
                    f"🔁 RELAY: Primary host failed. Attempting Dash Fallback | url={dash_url}"
                )
                try:
                    ...
                    if res_dash.status_code in [200, 201]:
                        ...
                        raise HTTPException(
                            ...
                            f"Relay Failed (All hosts) | error={res_dash.text}",
                        )
                except Exception as e2:
                    logger.error(
                        f"❌ RELAY: Critical DNS failure on all hosts | error={str(e2)}"
                    )
                    raise e2
            else:
...
async def admin_send_message(request: Request, background_tasks: BackgroundTasks):
    ...
    channel = data.get(
        "channel_source"
    )  # Removed default "whatsapp" to prevent logic bias
    ...
    if not tenant_id:
        if conv_id:
            tenant_id = await db.pool.fetchval(
                "SELECT tenant_id FROM chat_conversations WHERE id = $1", conv_id
            )

        if not tenant_id and phone:
            tenant_id = await db.pool.fetchval(
                "SELECT tenant_id FROM chat_conversations WHERE external_user_id = $1 LIMIT 1",
                phone,
            )
    ...
    if conv_id:
        conv_row = await db.pool.fetchrow(
            "SELECT id, meta, channel_source, external_user_id FROM chat_conversations WHERE id = $1",
            conv_id,
        )
        if conv_row and not phone:
            ...
            elif not channel and "whatsapp" in (
                conv_row["channel"] or ""
            ):  # Legacy fallback
                channel = "whatsapp"
    ...
    if not phone or not text:
        raise HTTPException(
            400, "Phone and message required (Could not resolve user from conversation)"
        )
...
async def get_media(media_id: str, current_user: User = Depends(get_current_user)):
    ...
    if not v_ycloud:
        ...
        v_ycloud_enc = await db.pool.fetchval(
            "SELECT value FROM credentials WHERE name = 'YCLOUD_API_KEY' AND scope = 'global' LIMIT 1"
        )
        if v_ycloud_enc:
            from utils import decrypt_password
    ...
    if not v_ycloud:
        raise HTTPException(
            status_code=500,
            detail="YCloud configuration missing (Not found in DB or ENV)",
        )
    ...
    async def iter_content():
        async with httpx.AsyncClient() as client:
            async with client.stream(
                "GET", target_url, headers={"X-API-Key": v_ycloud}
            ) as r:
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.get(
                target_url, headers={"X-API-Key": v_ycloud}, follow_redirects=True
            )
...
async def list_tools():
    ...
    return [
        {"name": "products_search", "type": "function", "service_url": "internal"},
        {
            "name": "order_lookup",
            "type": "tiendanube",
            "service_url": "api.tiendanube.com",
        },
...
async def analytics_summary(
    current_user: User = Depends(get_current_user), tenant_id: Optional[int] = None
):
    """
    ...
    try:
        ...
        blocked_convs = await db.pool.fetchval(
            "SELECT COUNT(*) FROM chat_conversations WHERE tenant_id = $1 AND status = 'human_override'",
            tid,
        )
        ...
        human_msgs = await db.pool.fetchval(
            "SELECT COUNT(*) FROM chat_messages WHERE tenant_id = $1 AND role = 'human_supervisor'",
            tid,
        )
...
async def setup_preflight(data: dict):
    ...
    try:
        ...
        logger.error("preflight_db_check_failed", extra={"error": str(e)})
        db_status = "FAIL"
...
async def test_openai():
    ...
    if not key or not key.startswith("sk-"):
        key_db = await db.pool.fetchval(
            "SELECT value FROM credentials WHERE name = 'OPENAI_API_KEY'"
        )
        if key_db:
            key = key_db
...
async def test_ycloud():
    ...
    if not key:
        key_db = await db.pool.fetchval(
            "SELECT value FROM credentials WHERE name = 'YCLOUD_API_KEY'"
        )
        if key_db:
            key = key_db
...
async def create_tenant(
    tenant: TenantModel, current_user: User = Depends(get_current_user)
):
    """Create or update a tenant."""
    if not current_user.is_verified:
        raise HTTPException(
            status_code=403, detail="Email verification required to create stores"
        )
    try:
        # Check if exists
        exists = await db.pool.fetchval(
            "SELECT id FROM tenants WHERE bot_phone_number = $1",
            tenant.bot_phone_number,
        )
...
async def update_tenant(
    ...
    existing = await db.pool.fetchrow(
        "SELECT id, bot_phone_number FROM tenants WHERE id = $1", tenant_id
    )
    if not existing:
        raise HTTPException(404, "Tenant not found")
    ...
    if new_phone and new_phone != existing["bot_phone_number"]:
        ...
        if duplicate:
            raise HTTPException(
                400, f"Phone number {new_phone} is already in use by another store"
            )
...
async def diagnostics_healthz():
    ...
    try:
        ...
        logger.error("diagnostics_db_check_failed", extra={"error": str(e)})
        db_status = "ERROR"

    # Check OpenAI & YCloud (usando funciones locales o mocks)
    return {"status": "OK", "checks": [{"name": "database", "status": db_status}]}
...
async def events_stream(limit: int = 10):
    ...
    rows = await db.pool.fetch(
        "SELECT * FROM chat_messages WHERE role = 'user' ORDER BY created_at DESC LIMIT $1",
        limit,
    )
    events = []
    for r in rows:
    ...
    out_rows = await db.pool.fetch(
        "SELECT * FROM chat_messages WHERE role='assistant' ORDER BY created_at DESC LIMIT $1",
        limit,
    )
    for r in out_rows:
        events.append(
...
async def get_analytics_kpis(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        msgs_today = await db.pool.fetchval(
            "SELECT COUNT(*) FROM chat_messages WHERE tenant_id = $1 AND created_at > CURRENT_DATE",
            tenant_id,
        )

        # 3. Active Users (Unique phones in last 24h)
        active_users = await db.pool.fetchval(
            "SELECT COUNT(DISTINCT from_number) FROM chat_messages WHERE tenant_id = $1 AND role='user' AND created_at > NOW() - INTERVAL '24 hours'",
            tenant_id,
        )
        ...
        try:
            errors_today = (
                await db.pool.fetchval(
                    "SELECT COUNT(*) FROM system_events WHERE tenant_id = $1 AND severity='ERROR' AND occurred_at > CURRENT_DATE",
                    tenant_id,
                )
                or 0
            )
            ...
            if current_user.role == "SuperAdmin":
                errors_today = (
                    await db.pool.fetchval(
                        "SELECT COUNT(*) FROM system_events WHERE severity='ERROR' AND occurred_at > CURRENT_DATE"
                    )
                    or 0
                )
...
async def report_assisted_gmv(
    ...
    try:
        ...
        try:
            ...
            logger.warning("roi_cache_write_failed", extra={"error": str(e)})
...
async def improve_prompt(req: PromptImproveRequest):
    ...
    try:
        ...
        openai_key = await get_tenant_credential_by_type(
            req.tenant_id, "OPENAI_API_KEY"
        )
        if not openai_key:
        ...
        if not openai_key:
            raise HTTPException(
                400,
                detail="Please configure your AI Credentials in Settings. No global key found.",
            )
        ...
        if req.field and req.field in FIELD_SYSTEM_PROMPTS:
            system_msg = FIELD_SYSTEM_PROMPTS[req.field].replace(
                "{user_input}", req.text
            )
        else:
...
async def list_agents():
    ...
    for row in rows:
        ...
        try:
            ...
            logger.warning("agent_config_json_parse_failed", extra={"error": str(e)})
            r["config"] = {}
...
async def get_telemetry_events(
    page: int = 1, page_size: int = 20, tenant_id: Optional[int] = None
):
    """
    ...
    base_query += (
        f" ORDER BY occurred_at DESC LIMIT ${len(args) + 1} OFFSET ${len(args) + 2}"
    )
...
async def ignite_engine(request: Request):
    ...
    if not tenant_id_phone or not store_name:
        ...
        if not tenant_id_phone:  # Strict requirement
            raise HTTPException(
                400, "Missing 'tenant_id' (Phone Number) or 'store_name'."
            )
    ...
    final_tn_token = (
        decrypt_password(row["tiendanube_access_token"])
        if row["tiendanube_access_token"]
        else None
    )
    ...
    store_website = row[
        "store_website"
    ]  # Already contains the best available (Payload or DB)
...
async def get_products(tenant_id: str):
    ...
    try:
        ...
        if tenant_id.isdigit() and len(tenant_id) < 6:
            ...
            row = await db.pool.fetchrow(
                "SELECT id FROM tenants WHERE bot_phone_number = $1 OR CAST(id AS TEXT) = $1",
                tenant_id,
            )
            if row:
        ...
        if not tenant_int_id:
            tenant_int_id = tenant_id  # Fallback
...
async def stream_engine_events_legacy(
    request: Request, tenant_id_phone: str, token: Optional[str] = None
):
    """
...
async def stream_engine_events(
    ...
    logger.info(
        "stream_connection_init", channel=channel, original_param=tenant_id_phone
    )
...
@router.get(
    "/onboarding/{tenant_id_phone}/status", dependencies=[Depends(verify_admin_token)]
)
@safe_db_call
...
async def get_business_assets(tenant_id: str):
    ...
    await redis_client.setex(
        cache_key, 5, json.dumps(assets)
    )  # Short TTL (5s) to allow updates during generation
    return assets
...
async def get_rag_galaxy(tenant_id: str):
    ...
    try:
        ...
        # 1. Fetch Assets
        rows = await db.pool.fetch(
            "SELECT asset_type, content FROM business_assets WHERE tenant_id = $1",
            tenant_id,
        )
        ...
        try:
            ...
            logger.warning("rag_galaxy_vector_count_failed", extra={"error": str(e)})
...
async def report_assistance_endpoint(req: ReportAssistanceRequest):
    ...
    try:
        ...
        """,
            ...
            json.dumps(
                {"reasoning": req.reasoning, "timestamp": datetime.now().isoformat()}
            ),
...
async def get_assist_audits(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        for r in rows:
            analysis = (
                json.loads(r["last_assist_analysis"])
                if r["last_assist_analysis"]
                else {}
            )
...
@router.get(
    "/rag/search", dependencies=[Depends(verify_internal_token)]
...
async def search_rag(
    ...
    try:
        ...
        if not effective_user_id:
            return {
                "ok": False,
                "error": "Missing user_id context for strict isolation",
            }

        openai_key = await get_tenant_credential_by_type(
            int(tenant_id), "OPENAI_API_KEY"
        )
        google_key = await get_tenant_credential_by_type(
            int(tenant_id), "GOOGLE_API_KEY"
        )
        ...
        rag = RAGCore(
            tenant_id, user_id=effective_user_id, api_key=api_key, provider=provider
        )
...
async def upload_knowledge_file(
    ...
    if not openai_key and not google_key:
        ...
        if not openai_key and not google_key:
            ...
            raise HTTPException(
                400,
                "Missing AI Credentials: No OpenAI or Google API keys found in vault.",
            )
    ...
    try:
        ...
        if "UndefinedColumn" in str(
            e
        ) or 'column "collection" of relation "rag_documents" does not exist' in str(e):
            logger.warning(
                "rag_schema_healing: Adding missing 'collection' column to rag_documents"
            )
            await db.pool.execute(
                "ALTER TABLE rag_documents ADD COLUMN IF NOT EXISTS collection VARCHAR DEFAULT 'General'"
            )
    ...
    return {"status": "processing", "id": str(doc_id), "filename": filename}


async def process_knowledge_ingestion(
    doc_id: str,
    tenant_id: int,
    user_id: str,
    file_path: str,
    ...
    try:
        ...
        if google_key:
            ...
            raise Exception(
                "Missing AI Credentials. No API keys found in vault or environment."
            )
        ...
        rag = RAGCore(
            str(tenant_id), user_id=user_id, api_key=api_key, provider=provider
        )
        ...
        if success:
            await db.pool.execute(
                "UPDATE rag_documents SET status = 'active' WHERE id = $1", doc_id
            )
            ...
            raise Exception(
                "RAG ingestion engine returned failure without specific error."
            )
        ...
        await db.pool.execute(
            "UPDATE rag_documents SET status = 'error', meta = $2 WHERE id = $1",
            doc_id,
            meta,
        )
    finally:
...
async def delete_knowledge_file(
    ...
    doc = await db.pool.fetchrow(
        "SELECT id, filename, tenant_id FROM rag_documents WHERE id=$1", doc_id
    )
    ...
    if doc.get("tenant_id") != current_user.tenant_id:
        raise HTTPException(
            403, "Access denied: You can only delete your own documents."
        )
    ...
    if supabase_url and supabase_key:
        logger.info(
            f"🌐 HTTP DELETE: Targeting vectors for ID {doc_uuid} via REST API..."
        )
        ...
        params = {"metadata->>source_id": f"eq.{doc_uuid}"}
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.delete(
                    target_endpoint, headers=headers, params=params, timeout=10.0
                )

                if response.status_code in [200, 204]:
                    logger.info(
                        f"✅ REST API SUCCESS: Vectors deleted via HTTP ({response.status_code})."
                    )
                else:
                    logger.error(
                        f"⚠️ REST API FAIL: {response.status_code} - {response.text}"
                    )
            except Exception as e:
    ...
    await db.pool.execute(
        "DELETE FROM rag_documents WHERE id=$1 AND tenant_id=$2",
        doc_id,
        doc["tenant_id"],
    )
...
class AgentModel(BaseModel):
    ...
    tenant_id: Optional[int] = (
        0  # 0 in frontend often sent for "Select...", but backend should enforce context
    )
    model_provider: str
    model_version: str
    temperature: float
    system_prompt_template: Optional[str] = (
        None  # Nexus v7.6: None to explicitly indicate not provided
    )
...
async def list_agents(current_user: User = Depends(get_current_user)):
    ...
    user_row = await db.pool.fetchrow(
        "SELECT tenant_id FROM users WHERE id = $1", current_user.id
    )
    if not user_row:
...
async def create_agent(
    agent: AgentModel, current_user: User = Depends(get_current_user)
):
    """
    ...
    if not current_user.email_verified:
        raise HTTPException(
            status_code=403, detail="Email verification required to create agents"
        )
    try:
        # Enforce Tenant ID
        ...
        if current_user.role == "SuperAdmin":
            target_tenant_id = (
                agent.tenant_id if agent.tenant_id and agent.tenant_id > 0 else None
            )
        ...
        if target_tenant_id:
            tenant_row = await db.pool.fetchrow(
                "SELECT id FROM tenants WHERE tenant_id = $1", target_tenant_id
            )
            if tenant_row:
        ...
        agent_id = await db.pool.fetchval(
            ...
            agent.temperature,
            clean_prompt,
            json.dumps(agent.enabled_tools),  # Use Sanizited Prompt
            json.dumps(agent.channels),
            agent.is_active,
            agent.template_type,
...
async def update_agent(
    agent_id: int, agent: AgentModel, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if is_template and current_user.role != "SuperAdmin":
            ...
            return {
                "status": "forked",
                "id": new_id,
                "message": "Agent cloned from template.",
            }
        ...
        prompt_to_use = (
            agent.system_prompt_template
            if agent.system_prompt_template is not None
        ...
        if not final_prompt:
            final_prompt = (
                f"You are a {agent.role} assistant. Help customers professionally."
            )
...
async def delete_agent(agent_id: int, current_user: User = Depends(get_current_user)):
    ...
    """
    logger.info(
        f"🗑️ DELETE request for Agent {agent_id} | User: {current_user.email} (Solving via User Table)"
    )
    ...
    try:
        ...
        if not user_row:
            ...
            logger.error(
                f"⛔ User Resolution Failed: UUID {current_user.id} not found in DB."
            )
            raise HTTPException(
                status_code=401, detail="User identity verification failed."
            )

        real_tenant_id = user_row["tenant_id"]
        logger.info(
            f"✅ Resolved Tenant ID from User Table: {real_tenant_id} (Integer)"
        )
        ...
        if result == "DELETE 0":
            logger.warning(
                f"⚠️ Agent {agent_id} not found for Tenant ID {real_tenant_id}"
            )
            # Verificamos si existe el agente para dar mejor error
            exists = await db.pool.fetchval(
                "SELECT id FROM agents WHERE id = $1", int(agent_id)
            )
            if exists:
                raise HTTPException(
                    403, "Access denied: Agent belongs to another tenant"
                )
            else:
                raise HTTPException(404, "Agent not found")
...
async def get_agent_config(agent_id: int):
    ...
    if not data.get("model_provider"):
        data["model_provider"] = "openai"
    if not data.get("model_version"):
        data["model_version"] = "gpt-4o"
...
async def simulate_agent(req: AgentSimulation):
    ...
    try:
        ...
        google_key = await get_tenant_credential_by_type(
            req.tenant_id, "GOOGLE_API_KEY"
        )
        tn_token = await get_tenant_credential_by_type(
            req.tenant_id, "TIENDANUBE_ACCESS_TOKEN"
        )
        ...
        agent_request = {
            ...
            "agent_config": {
                **agent_config,
                ...
                "reasoning_effort": req.formData.get(
                    "reasoning_effort"
                ),  # Nexus v5.99: GPT-5.2 advanced param
            },
            "credentials": {
                "openai_api_key": final_openai_key,
                "google_api_key": google_key or os.getenv("GOOGLE_API_KEY"),
                "tiendanube_store_id": str(tenant["tiendanube_store_id"])
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    return {
                        "status": "error",
                        "response": f"Error del Agente: {err_text.decode()}",
                    }
                ...
                async for chunk in resp.aiter_lines():
                    ...
                    try:
                        ...
                        if etype == "token":
                            ...
                            return {
                                "status": "error",
                                "response": f"Error del Agente: {evt.get('content', 'Unknown error')}",
                            }
                    except json.JSONDecodeError:
        ...
        if not full_response:
            ...
            return {
                "status": "success",
                "response": "⚠️ El agente no generó ninguna respuesta. Revisa tu configuración o el catálogo.",
            }
    ...
    try:
        ...
        for row in rows:
            ...
            if isinstance(config, str):
                config = json.loads(config)  # Double decoding safety

            # Map to Frontend Structure
            final_templates[key] = {
                "label": row["name"],  # The template name
                "description": config.get("store_description")
                or config.get("description")
                or "Plantilla personalizada",
...
@router.get(
    "/agents/sales-config/{tenant_id_ignored}",
    dependencies=[Depends(verify_admin_token)],
)
async def get_sales_agent_config(
    tenant_id_ignored: str,
    current_user: User = Depends(get_current_user),
    ...
    user_row = await db.execute(
        text("SELECT tenant_id FROM users WHERE id = :id"), {"id": current_user.id}
    )
...
async def receive_chatwoot_webhook(
    request: Request, background_tasks: BackgroundTasks, access_token: str
):
    """
    ...
    try:
        ...
        logger.warning("chatwoot_webhook_json_parse_failed", extra={"error": str(e)})
    ...
    msg_type = payload.get(
        "message_type"
    )  # 'incoming' (User) vs 'outgoing' (Agent/Human)
    ...
    if "whatsapp" in cw_lower:
        nexus_channel = "whatsapp"
    elif "instagram" in cw_lower:
        nexus_channel = "instagram"
    elif "facebook" in cw_lower:
        nexus_channel = "facebook"
    ...
    try:
        ...
        logger.warning(
            f"⚠️ UPSERT: Primary conflict, falling back to ordered resolution | error={str(e)}"
        )
        ...
        conv_row_fallback = await db.pool.fetchrow(
            conv_query,
            tenant_id,
            nexus_channel,
            identifier,
        ...
        if conv_row_fallback:
            ...
            await db.pool.execute(
                "UPDATE chat_conversations SET meta = meta || $1::jsonb, updated_at = NOW() WHERE id = $2",
                json.dumps(meta_data),
                conversation_id,
            )
        else:
    ...
    """,
        ...
        data,
        identifier,
        json.dumps(parsed_attachments),
    )

    # 5. Integrate with AI Orchestrator (Lifecycle v6.2)
    is_echo = msg_type == "outgoing"

    # CRITICAL FIX: Do not buffer outgoing messages (Echoes)
    # This prevents the Agent from replying to itself (Infinite Loop Fix)
    if is_echo:
        logger.info(
            f"🚫 ECHO: Skipping AI processing for outgoing message | msg_id={msg_id}"
        )
    else:
        ...
        logger.info(
            f"📨 WEBHOOK: Received incoming message | identifier={identifier} | tenant={tenant_id} | conv={conversation_id} | content={data[:50]}..."
        )
...
async def get_integration_status(current_user: User = Depends(get_current_user)):
    ...
    async def fetch_creds():
        return await db.pool.fetch(
            "SELECT category, name FROM credentials WHERE tenant_id = $1", tenant_id
        )
    ...
    status = {"whatsapp": False, "instagram": False, "facebook": False, "web": True}
...
async def save_web_widget_config(
    config: dict, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if requested_tenant_id:
            ...
            if current_user.role == "SuperAdmin":
                ...
                logger.warning(
                    "security_tenant_injection_attempt",
                    user_id=current_user.id,
                    requested=requested_tenant_id,
                )
        ...
        payload = {"tenant_id": target_tenant_id, "redirect_uri": redirect_uri}
        if code:
            payload["code"] = code
        if access_token:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code != 200:
                logger.error(
                    f"meta_service_connect_failed: {resp.status_code} - {resp.text}"
                )
...
async def update_contact_circle(
    ...
    if circle not in valid_circles:
        raise HTTPException(
            status_code=400, detail=f"Invalid circle. Must be one of {valid_circles}"
        )
...
async def rag_shadow_search(
    q: str, tenant_id: int, user_id: Optional[str] = None, circle: Optional[str] = None
):
    """
    ...
    try:
        ...
        results = []
        for d in docs:
            results.append({"content": d.page_content, "metadata": d.metadata})

...
async def get_current_tenant_webhook(
    ...
    if target_tenant_id:
        try:
            result = await db.execute(
                select(Tenant).where(Tenant.id == int(target_tenant_id))
            )
            tenant_orm = result.scalar_one_or_none()
            if tenant_orm:
                if not tenant_orm.is_active:
    ...
    if body.get("provider") == "chatwoot":
        raw_channel_id = str(
            body.get("meta", {}).get("chatwoot_inbox_id") or body.get("inbox_id") or ""
        )
    ...
    if not raw_channel_id:
        try:
            ...
            logger.warning("facebook_metadata_extraction_failed", error=str(e))
    ...
    if raw_channel_id:
        ...
        if external_account_id:
            query = text(
                "SELECT tenant_id FROM channel_bindings WHERE channel_id = :cid AND (external_account_id = :aid OR external_account_id IS NULL) LIMIT 1"
            )
            res = await db.execute(
                query, {"cid": str(raw_channel_id), "aid": str(external_account_id)}
            )
        else:
            query = text(
                "SELECT tenant_id FROM channel_bindings WHERE channel_id = :cid LIMIT 1"
            )
    ...
    if clean_phone:
        result = await db.execute(
            select(Tenant).where(Tenant.bot_phone_number == clean_phone)
        )
        tenant_orm = result.scalar_one_or_none()
    ...
    if not tenant_orm:
        logger.error(
            ...
            identifier=raw_channel_id,
        )
        raise HTTPException(
            status_code=404, detail=f"Tenant not found for identifier {raw_channel_id}"
        )
    ...
    logger.info(
        "tenant_resolved", tenant_id=tenant_data.id, store=tenant_data.store_name
    )
    return tenant_data

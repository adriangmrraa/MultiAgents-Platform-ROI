...
from sqlalchemy import String, Text, Boolean, Integer, Float, ForeignKey, DateTime, CheckConstraint, UniqueConstraint
...
class Plan(Base, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(50), unique=True, nullable=False)  # free, pro, enterprise
    display_name: Mapped[str] = mapped_column(String(100), nullable=False)  # "Free", "Pro", "Enterprise"
    ...
    billing_period: Mapped[str] = mapped_column(String(20), default="monthly")  # monthly, yearly
    ...
    max_tokens_per_month: Mapped[int] = mapped_column(Integer, default=50000)  # LLM tokens
...
class Subscription(Base, TimestampMixin):
    ...
    # Payment provider
    payment_provider: Mapped[Optional[str]] = mapped_column(String(30))  # stripe, mercadopago
    external_subscription_id: Mapped[Optional[str]] = mapped_column(String(255))  # Stripe sub_xxx / MP preapproval_xxx
    external_customer_id: Mapped[Optional[str]] = mapped_column(String(255))  # Stripe cus_xxx / MP customer

    # Relationships
    plan: Mapped["Plan"] = relationship("Plan", back_populates="subscriptions")
    tenant: Mapped["Tenant"] = relationship("Tenant", backref="subscription", uselist=False)
...
class Invoice(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id"), nullable=False, index=True)
    subscription_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("subscriptions.id"))

...
async def read_users_me(
        ...
        if sub:
            ...
            sub_info = {
                ...
                "trial_days_remaining": max(
                    0, (sub.trial_ends_at.replace(tzinfo=None) - datetime.utcnow()).days
                )
                if sub.trial_ends_at
                else None,
            }
    ...
    return {
        ...
        "is_verified": current_user.is_verified,
        "store_name": sanitize_html(current_user.tenant.store_name)
        if current_user.tenant
        else None,
        "full_name": sanitize_html(current_user.full_name)
        if current_user.full_name
        else None,
        "avatar_url": sanitize_html(current_user.avatar_url)
...
async def register(
    ...
    is_valid, message = AccountSecurity.validate_password_strength(user_in.password)
    if not is_valid:
    ...
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=400,
            detail="Phone number already registered. Please provide unique.",
        )
    ...
    try:
        ...
        if "store_name" in str_e:  # If unique constraint exists
            raise HTTPException(
                status_code=400, detail="Store name already registered."
            )
        if "users_email_key" in str_e or "unique constraint" in str_e:  # Fallback
            raise HTTPException(
                status_code=400, detail="Resource already exists (Email or Phone)."
            )
    ...
    try:
        ...
        if free_plan:
            ...
            await db.commit()
            logger.info(
                "trial_subscription_created", tenant_id=new_tenant.id, trial_days=10
            )
    ...
    try:
        ...
        await EmailService.send_verification_email(
            new_user.email, verification_token, tenant_id=new_user.tenant_id
        )
...
async def login(
    ...
    # 0. Check lockout
    try:
        locked = await account_security.check_login_attempts(user_in.email)
        if locked:
            remaining = await account_security.get_lockout_remaining(user_in.email)
            if remaining:
                minutes = remaining // 60
                seconds = remaining % 60
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail=f"Account locked due to too many failed login attempts. Please try again in {minutes}m {seconds}s.",
                )
            else:
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
    ...
    if not user or not security.verify_password(user_in.password, user.password_hash):
        # Record failed login
        try:
            await account_security.record_failed_login(user_in.email)
        except Exception as e:
            logger.error("redis_record_failed_login_failed", error=str(e))
        raise HTTPException(status_code=401, detail="Incorrect email or password")

    # 2. Successful login: reset failed logins
    try:
        await account_security.reset_failed_logins(user_in.email)
    except Exception as e:
        logger.error("redis_reset_failed_logins_failed", error=str(e))

    # 3. Zero Trust Guard -> MODIFIED: Allow login for Spectator Mode
...
async def resend_verification(
    ...
    if current_user.is_verified:
        logger.info(
            "resend_verification_skipped_already_verified", user_id=str(current_user.id)
        )
    ...
    if current_user.last_verification_email_at:
        ...
        logger.info(
            "resend_verification_cooldown_check", seconds_passed=delta.total_seconds()
        )
        if delta.total_seconds() < 60:
            logger.warning(
                "resend_verification_cooldown_block",
                seconds_remaining=60 - int(delta.total_seconds()),
            )
            raise HTTPException(
                status_code=429,
    ...
    if user:
        ...
        user = User(
            ...
            password_hash=security.get_password_hash(
                uuid.uuid4().hex
            ),  # Random password (Google auth only)
            tenant_id=new_tenant.id,
            role="owner",
...
async def forgot_password(
    request: Request, data: ForgotPasswordSchema, db: AsyncSession = Depends(get_db)
):
    """
...
async def reset_password(request: Request, data: ResetPasswordSchema, db: AsyncSession = Depends(get_db)):
    ...
    # Validate password strength
    is_valid, message = AccountSecurity.validate_password_strength(data.new_password)
    if not is_valid:
        raise HTTPException(status_code=400, detail=message)
...
async def update_profile(
    ...
    if data.full_name is not None:
        current_user.full_name = sanitize_html(data.full_name)

    if data.avatar_url is not None:
        current_user.avatar_url = sanitize_html(data.avatar_url)

    if data.password:
        # Validate password strength
        is_valid, message = AccountSecurity.validate_password_strength(data.password)
        if not is_valid:
            raise HTTPException(status_code=400, detail=message)

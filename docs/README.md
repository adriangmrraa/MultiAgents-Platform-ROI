# 📚 Nexus Platform Documentation Hub

Welcome to the documentation center for Nexus v5.1 "Sovereign".

## 🚀 Architecture & Vision
Start here to understand the high-level design.
*   [**NEXUS_ARCHITECTURAL_BLUEPRINT.md**](NEXUS_ARCHITECTURAL_BLUEPRINT.md) - The 5 Pillars of Sovereign Architecture.
*   [**TECHNICAL_DEEP_DIVE_V5_1.md**](TECHNICAL_DEEP_DIVE_V5_1.md) - Detailed breakdown of the Vault, Meta Uplink, and Hybrid SMTP.
*   [**NEXUS_BUSINESS_STRATEGY_VISION.md**](NEXUS_BUSINESS_STRATEGY_VISION.md) - The "Why" behind the "Business Factory" concept.

## 🦅 Operational Guides
For Administrators and DevOps.
*   [**Manual de Vuelo Nexus v5.md**](Manual%20de%20Vuelo%20Nexus%20v5.md) - **(Start Here)** Daily Operations Manual.
*   [**WORKFLOW_GUIDE.md**](WORKFLOW_GUIDE.md) - How to run standard procedures.
*   [**DATABASE_EVOLUTION_GUIDE.md**](DATABASE_EVOLUTION_GUIDE.md) - Schema migration and history.

## 🔬 Technical Logic Deep Dives
Atomic documentation for specific modules. (References for Developers)

### Frontend & Agents
*   [**MAGIC_LOGIC_DEEP_DIVE.md**](MAGIC_LOGIC_DEEP_DIVE.md) - The "Magic" Onboarding Flow (7 Agents).
*   [**CHATS_LOGIC_DEEP_DIVE.md**](CHATS_LOGIC_DEEP_DIVE.md) - Hybrid Sync & Messaging.
*   [**AGENTS_LOGIC_DEEP_DIVE.md**](AGENTS_LOGIC_DEEP_DIVE.md) - Agent Configuration & CRUD.
*   [**FORGE_LOGIC_DEEP_DIVE.md**](FORGE_LOGIC_DEEP_DIVE.md) - Business Asset Editor & Fusion.
*   [**ANALYTICS_LOGIC_DEEP_DIVE.md**](ANALYTICS_LOGIC_DEEP_DIVE.md) - SuperAdmin Telemetry.

### Infrastructure
*   [**SETTINGS_LOGIC_DEEP_DIVE.md**](SETTINGS_LOGIC_DEEP_DIVE.md) - Credentials & Encryption.
*   [**STORES_LOGIC_DEEP_DIVE.md**](STORES_LOGIC_DEEP_DIVE.md) - Tenant Lifecycle.
*   [**INTEGRATIONS_LOGIC_DEEP_DIVE.md**](INTEGRATIONS_LOGIC_DEEP_DIVE.md) - Meta & YCloud.
*   [**RAG_LOGIC_DEEP_DIVE.md**](RAG_LOGIC_DEEP_DIVE.md) - Vector Database & Ingestion.

## 📜 Archives & Specifications
*   `FRONTEND_SPECIFICATION.md` / `BACKEND_SPECIFICATION.md`: API Contracts.
*   `requirements_v1.md`: Original v1 Concept (Legacy).
